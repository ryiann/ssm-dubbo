...
public class StringUtil {
	...
	/**
	 * 将对象转换成xml格式的字符串，但对于参数直接是基础类型将直接输出.
	 * 
	 * @param objectValue
	 *            Object
	 * @return String
	...
	public static String ObjectToString(Object objectValue) {
		...
		if (objectValue instanceof Iterable || objectValue instanceof Map) {
			appendValue("records", objectValue, logStr, objectValue.getClass());
			return logStr.toString();
		}
		if (isBaseTypeClass(objectValue) || objectValue.getClass().getName().indexOf("com.howbuy") != 0) {
			return "" + objectValue;
		}

		Class targetClass = objectValue.getClass();
		for (; !targetClass.equals(Object.class); targetClass = targetClass.getSuperclass()) {

			Field[] fields = (targetClass).getDeclaredFields();

			for (Field field : fields) {
				String name = field.getName();
				if (name.equalsIgnoreCase("serialVersionUID") || Modifier.isStatic(field.getModifiers())
				...
				try {
					...
					if (field.getType() == Boolean.class || field.getType() == boolean.class) {
						method = objectValue.getClass().getMethod(
								"is" + name.substring(0, 1).toUpperCase() + name.substring(1), null);
					} else {
						method = objectValue.getClass().getMethod(
					...
					try {
						...
						Object value = method.invoke(objectValue, null);
						appendValue(name, value, logStr, field.getType());
						// }
	...
	protected static void appendValue(String name, Object value, StringBuilder logStr, Class paramClass) {
		logStr.append("<").append(name).append(">");
		if (isBaseTypeClass(value)) {
			logStr.append(value);
		} else if (value instanceof Collection) {
			for (Iterator ite = ((Collection) value).iterator(); ite.hasNext();) {
				Object iteValue = ite.next();
				if (isBaseTypeClass(iteValue)) {
					logStr.append("<").append(name).append(">");
					logStr.append(value);
					logStr.append("</").append(name).append(">");
				} else {
					logStr.append(ObjectToString(iteValue));
				}
			}
			...
			for (Iterator ite = keySet.iterator(); ite.hasNext();) {
				Object key = ite.next();// map key 不能是对象
				appendValue(key.toString(), map.get(key), logStr, map.get(key).getClass());
			}
		} else {
			logStr.append(ObjectToString(value));
	...
	public static void main(String[] args) {
		String x = calDigitPlusOne("002000", 7);
...
//
//		System.out.println(StringUtil.ObjectToString(list));
		
//		 System.out.println(StringUtil.point("1236"));
