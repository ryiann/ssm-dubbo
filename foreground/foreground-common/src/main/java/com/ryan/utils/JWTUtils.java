...
import com.ryan.constants.Constants;
...
public class JWTUtils {
	...
	static {
		...
		priKey = RSAUtils.getPrivateKey(modulus, privateExponent);//私钥
		pubKey = RSAUtils.getPublicKey(modulus, publicExponent);//公钥
	}
	...
	public static String setTokenExpiration(String uid) {
		long endTime = new Date().getTime()-1000 * 60;
		return Jwts.builder().setSubject(uid).setExpiration(new Date(endTime))
				.signWith(SignatureAlgorithm.RS512, priKey).compact();
	}
